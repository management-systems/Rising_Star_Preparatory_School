...
export default function AdminAdmissions() {
  ...
  const [form, setForm] = useState({ childName: '', parentName: '', phone: '', age: '', program: 'Playgroup' });
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    setForm({ childName: '', parentName: '', phone: '', age: '', program: 'Playgroup' });
    setShowForm(false);
  };
  ...
  return (
    <AdminShell title="Admissions">
      <div className="flex items-center justify-between mb-6">
        <p className="text-sm text-gray-500">{admissions.length} total records</p>
        <button onClick={() => setShowForm(!showForm)} className="px-4 py-2.5 bg-[#2B4C7E] text-white rounded-lg text-sm font-medium hover:bg-[#1a365d] transition">
          {showForm ? 'Cancel' : '+ Add Admission'}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">New Admission Record</h3>
          <form onSubmit={handleAdd} className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Child&apos;s Name *</label>
              <input type="text" required value={form.childName} onChange={(e) => setForm({...form, childName: e.target.value})} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#2B4C7E]" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Parent Name *</label>
              <input type="text" required value={form.parentName} onChange={(e) => setForm({...form, parentName: e.target.value})} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#2B4C7E]" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Phone *</label>
              <input type="tel" required value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#2B4C7E]" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Child&apos;s Age *</label>
              <input type="text" required placeholder="e.g. 3 years" value={form.age} onChange={(e) => setForm({...form, age: e.target.value})} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#2B4C7E]" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Program *</label>
              <select value={form.program} onChange={(e) => setForm({...form, program: e.target.value})} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#2B4C7E]">
            ...
            <div className="flex items-end">
              <button type="submit" className="px-5 py-2.5 bg-[#2B4C7E] text-white rounded-lg text-sm font-medium hover:bg-[#1a365d] transition">
                Save Record
              </button>
            </div>
          </form>
      ...
      {admissions.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-xl">
            ...
            <p className="text-sm text-gray-500">No admission records yet</p>
            <p className="text-xs text-gray-400 mt-1">Click &quot;+ Add Admission&quot; to create the first record</p>
          </div>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-left font-semibold text-gray-600">Child</th>
                ...
                {admissions.map((a) => (
                  <tr key={a.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-800">{a.childName}</p>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${a.status === 'Approved' ? 'bg-green-50 text-green-700' : a.status === 'Rejected' ? 'bg-red-50 text-red-700' : 'bg-yellow-50 text-yellow-700'}`}>{a.status}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        {a.status === 'Pending' && (
                          <>
                            <button onClick={() => updateStatus(a.id, 'Approved')} className="text-xs text-green-600 hover:text-green-800 font-medium">Approve</button>
                            <button onClick={() => updateStatus(a.id, 'Rejected')} className="text-xs text-red-600 hover:text-red-800 font-medium">Reject</button>
                          </>
                        )}
                        <button onClick={() => handleDelete(a.id)} className="text-xs text-gray-400 hover:text-red-600 font-medium">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
...
function AdminShell({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  const pathname = '/admin/admissions';
  const sidebarItems = [
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 bg-white border-r border-gray-200">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {sidebarItems.map((item) => (
            <Link key={item.label} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition ${pathname === item.href ? 'bg-[#2B4C7E]/5 text-[#2B4C7E]' : 'text-gray-600 hover:bg-gray-50'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-1.5 text-gray-500"><svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg></button>
            <h1 className="text-lg font-semibold text-gray-800">{title}</h1>
          </div>
          <div className="w-9 h-9 bg-[#2B4C7E] rounded-full flex items-center justify-center"><span className="text-white text-xs font-bold">MS</span></div>
        </header>
